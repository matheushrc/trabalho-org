...
\begin{frame}{Metodologia --- adaptação do código}
  \begin{itemize}
    ...
    \item Simulações em ambiente Docker (gem5.opt + \texttt{inorder.py}), CPU
          \texttt{TIMING} a clock fixo de 3\,GHz, L1 32\,kB, L2 256\,kB,
          DDR3 1\,GB.
  \end{itemize}
\end{frame}

\begin{frame}{Metodologia --- geração assistida das simulações}
  \begin{itemize}
    \item \texttt{inorder.py} parametrizado (via IA) com \texttt{argparse}:
          CPU, preditor, clock, caches L1/L2, hierarquia, núcleos, memória ---
          usado para fixar os valores padrão (TIMING, 3\,GHz, L1 32\,kB,
          L2 256\,kB, 1\,GB) na comparação dos preditores.
