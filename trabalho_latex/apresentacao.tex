...
% 20 frames (seminário original tinha 21; -1 por mesclar dois slides de metodologia).
...
\begin{frame}{Metodologia --- ambiente e geração assistida}
  \begin{itemize}
    \item Simulações em ambiente Docker (gem5.opt + \texttt{inorder.py}),
          parametrizado (via IA) com \texttt{argparse}: CPU, preditor,
          clock, caches L1/L2, hierarquia, núcleos, memória.
    \item Valores fixados na comparação dos preditores: CPU \texttt{TIMING}
          (em-ordem), 3\,GHz, L1 32\,kB, L2 256\,kB, DDR3 1\,GB.
    \item Comparação executada com o vetor padrão ($n=6$) e, para validar em
          escala maior, também com $n=100.000$ elementos (gerado pela IA em
          assembly puro a partir de \texttt{heapSort\_gem5.s}).
  \end{itemize}
\end{frame}
  ...
  \begin{columns}[t]
    \column{0.5\textwidth}
      ...
      \begin{itemize}
        ...
        \item \textbf{Cenário A} --- LocalBP (local simples)
...
\begin{frame}{Metodologia --- métricas coletadas}
  ...
  \begin{itemize}
    \item \texttt{branchPred.BTBLookups} (consultas à Branch Target Buffer)
          --- total de desvios avaliados;
    \item \texttt{branchPred.condIncorrect} (desvios condicionais
          incorretos) --- predições incorretas;
    \item \texttt{simTicks} --- tempo total de simulação;
    \item \textbf{IPC} (instruções por ciclo) =
          \texttt{simInsts / core\_numCycles} (instruções simuladas /
          ciclos do núcleo).
  \end{itemize}
  \vfill
  Taxa de erro global = \texttt{condIncorrect / BTBLookups}
  (predições incorretas sobre o total de desvios avaliados).
\end{frame}
