...
\title{Avalia{\c{c}}{\~a}o de Predi{\c{c}}{\~a}o de Desvios no HeapSort em {RISC-V} com o Simulador gem5}

\author{Matheus Henrique Rodrigues da Costa\inst{1},
  Lucas Smaniotto Schuch\inst{1}\\[4pt]
  Kassiara Regina Bosing\inst{1},
  Jean Carlos Canova Gondorek\inst{1}\\[4pt]
  Jonathan Gotz Correa\inst{1}}

\address{Curso de Ci{\^e}ncia da Computa{\c{c}}{\~a}o -- Universidade Federal da Fronteira Sul (UFFS)\\
  Chapec{\'o} -- SC -- Brasil\\[6pt]
  {\footnotesize
  \texttt{matheus.dacosta@estudante.uffs.edu.br}\\
  \texttt{lucas.schuch@estudante.uffs.edu.br}\\
  \texttt{kassiara.bosing@estudante.uffs.edu.br}\\
  \texttt{jean.gondorek@estudante.uffs.edu.br}\\
  \texttt{jonathan.correa@estudante.uffs.edu.br}}
}
...
\begin{verbatim}
riscv64-linux-gnu-gcc -static heapSort_original.s -o heapSort_original.riscv
\end{verbatim}
...
Sem saber de antemão quais combinações de parâmetros cobririam os diferentes aspectos da microarquitetura, pedimos a um modelo de LLM que reescrevesse \texttt{inorder.py} aceitando, via \texttt{argparse}, CPU, preditor de desvio, clock, caches L1/L2, hierarquia, núcleos e memória, e gerasse \texttt{run\_all\_simulations.sh} com uma varredura uniparamétrica (um eixo por vez), origem das pastas \texttt{simulations/\{branch\_prediction, cpu\_type, clock\_freq, cache\_l1, cache\_l2, cache\_hierarchy, num\_cores, memory, hybrids\}}.

Percebemos que vários eixos (L2, memória, núcleos, hierarquia) não variavam o desempenho, pois o vetor de $n=6$ cabia todo na L1. Pedimos então à IA que reescrevesse \texttt{heapSort\_gem5.s} para gerar, em assembly puro, um vetor aleatório de 100.000 elementos. Inspirados em \texttt{GridSearchCV}/\texttt{RandomizedSearchCV}, geramos também \texttt{search\_simulations.py}, com busca em grade e aleatória sobre o espaço de hardware (pastas \texttt{simulations/search/grid\_*} e \texttt{random\_*}).
...
\begin{itemize}
  \item \texttt{branchPred.lookups} --- total de desvios condicionais avaliados;
...
Ferramentas de inteligência artificial foram utilizadas na reescrita de \texttt{inorder.py} e do HeapSort em escala (\texttt{heapSort\_100k.s}), na geração dos scripts de automação e busca de hiperparâmetros (\texttt{run\_all\_simulations.sh}, \texttt{search\_simulations.py}) e na revisão textual deste relatório, conforme descrito na Seção~3.2. A interpretação dos resultados, a execução dos experimentos e a análise microarquitetural são de responsabilidade dos autores.
