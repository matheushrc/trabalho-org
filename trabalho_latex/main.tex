...
\subsection{Cenário padrão (3\,GHz, pipeline em-ordem)}

A entrega parcial confirmou a execução correta do binário no gem5. A Tabela~\ref{tab:baseline} resume as métricas do cenário padrão (LocalBP) com CPU em-ordem a 3\,GHz.

\begin{table}[ht]
\centering
\caption{Métricas do cenário padrão (CPU em-ordem, 3\,GHz, $n=6$, LocalBP)}
\label{tab:baseline}
\begin{tabular}{lc}
\toprule
...
# Simular no gem5 (CPU em-ordem, cenario padrao)
